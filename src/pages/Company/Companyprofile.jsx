...
import {
  MdLocationOn, MdPhone, MdLink,
  MdEdit, MdCameraAlt, MdVerified,
} from "react-icons/md";
...
export default function CompanyProfile() {
  ...
  // ── SuperAdmin has no company — redirect them away immediately ──────────
  useEffect(() => {
    if (isSuperAdmin) {
  ...
  const getGroupedHours = () => {
    ...
    for (let i = 1; i <= sortedHours.length; i++) {
      ...
      if (currentTime === prevTime && i < sortedHours.length) {
        ...
        if (currentDay) { startDay = currentDay; lastDay = currentDay; }
      }
    }
    return groups;
  };
  ...
  // Block render for superadmin (redirect fires above, this is just a safety net)
  ...
  if (!company) return (
    <Box p={10}>
      <Alert status="error"><AlertIcon />No Company Data Found</Alert>
    </Box>
  );
  ...
  return (
    <Box maxW="1000px" mx="auto" pb={10} p={{ base: 4, md: 8 }}>
      ...
      {/* Header */}
      <Box bgGradient="linear(to-r, purple.600, blue.600)" h="160px" borderRadius="2xl"
        position="relative" mb="80px" boxShadow="lg">
        <Flex
          position="absolute" bottom="-40px"
          left={{ base: "50%", md: "40px" }}
          transform={{ base: "translateX(-50%)", md: "none" }}
          direction={{ base: "column", md: "row" }} align="flex-end" gap={6}
          w={{ base: "full", md: "auto" }} px={{ base: 4, md: 0 }}
        >
          <Box position="relative">
            ...
            <IconButton
              icon={<MdCameraAlt />} position="absolute" bottom="5px" right="5px"
              colorScheme="purple" rounded="full" size="sm"
      ...
      <SimpleGrid columns={{ base: 1, md: 2 }} spacing={8}>
        ...
        {/* Contact Info */}
        <VStack align="stretch" spacing={6} bg={cardBg} p={8} borderRadius="2xl"
          border="1px solid" borderColor={borderClr} boxShadow="sm">
          <Heading size="xs" color="purple.500" textTransform="uppercase" letterSpacing="widest">
            Contact Information
          </Heading>
          <VStack align="stretch" spacing={5}>
            <HStack spacing={4}>
              ...
              <Box>
                <Text fontSize="xs" color={subColor}>Address</Text>
                <Text fontSize="sm" fontWeight="600">{company?.address || "Not specified"}</Text>
              </Box>
            </HStack>
            <HStack spacing={4}>
              <Icon as={MdPhone} color="purple.400" fontSize="20px" />
              <Box>
                <Text fontSize="xs" color={subColor}>Phone</Text>
                <Text fontSize="sm" fontWeight="600">{company?.phone || "Not specified"}</Text>
              </Box>
            </HStack>
            <HStack spacing={4}>
              <Icon as={MdLink} color="purple.400" fontSize="20px" />
              <Box>
                <Text fontSize="xs" color={subColor}>Website</Text>
                <Text fontSize="sm" fontWeight="600" color="blue.500">{company?.website || "None"}</Text>
              </Box>
            </HStack>
          </VStack>
        </VStack>
        ...
        <VStack align="stretch" spacing={6} bg={cardBg} p={8} borderRadius="2xl"
          ...
          <Box>
            {getGroupedHours().map((group, index, arr) => (
              <Box key={index}>
                ...
                {index < arr.length - 1 && <Divider borderColor={borderClr} />}
              </Box>
            ))}
          </Box>
        </VStack>
